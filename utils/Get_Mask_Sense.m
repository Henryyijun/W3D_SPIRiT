...
col_low = size_sense(2)/2- col_size/2 + 1;
col_high = size_sense(2)/2 + col_size/2;
% 1. row_size is an 2-element vector, gc is the range of the ACS line(row)
if size(row_size, 2) == 2
    % mask_sense(row_size(1):row_size(2),col_low:col_high) = 1;
    mask_sense(row_size(1):row_size(2),:) = 1;
